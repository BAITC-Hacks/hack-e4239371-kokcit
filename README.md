...
- получает доступный на момент выпуска архивный прогноз погоды;
...
метрики моделей, историю запусков и почасовую таблицу. Результат можно скачать
в CSV.
...
7. Скачайте CSV и откройте рассчитанный запуск из истории.
...
уникальных почасовых значения для каждой турбины. Перед экспортом система
проверяет полноту временной сетки и отсутствие дубликатов.
...
отчёт с метриками. Для первой загрузки февральской погоды требуется доступ к
Open-Meteo; после сохранения расчёт можно повторять offline.
...
| `GET` | `/api/forecasts/{run_id}/export.csv` | Экспорт одного прогноза |
| `GET` | `/api/forecasts/february/export.csv` | Экспорт полного месяца |
